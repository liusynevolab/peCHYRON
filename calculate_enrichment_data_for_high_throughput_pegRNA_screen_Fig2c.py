...
for line in hpm_file:
    ...
    hpm_upper = stripped_hpm_info[0].upper() #grabbing the 20 nt variable sequence from the pegRNA plasmid
    hpm_upper_seq = Seq(hpm_upper) #converting the 20 nt variable sequence to a "Seq" object
    hpm_insertion_seq = hpm_upper_seq.reverse_complement() #taking the reverse complement of the 20 nt variable sequence, to get the insertion sequence
    if hpm_insertion_seq not in hpm_dict:	
        hpm_dict[hpm_insertion_seq] = stripped_hpm_info[1]
    else:
...
final_df.to_csv('PER17_dataframe_counted_insertions.csv') 
